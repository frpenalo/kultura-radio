...
function djPersonaTts(kind: string): any {
  ...
  // Kultura: las capsulas (noticias/transito/etc.) usan voz bilingue OpenAI;
  // el DJ (dj-speak/link/idents) se queda en kokoro.
  if (kind === 'capsule') return { engine: 'cloud', cloudProvider: 'elevenlabs', voice: 'rEVYTKPqwSMhytFPayIb' };
...
function normalizeForSpeech(text: string) {
  ...
  return text.replace(/\bSUB\s*(?:\/|slash)\s*WAVE\b/gi, 'Subwave')
    // Quita Markdown para que el TTS no lo lea en voz alta (ej. "**" => "asterisco").
    .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')  // [texto](url) -> texto
    .replace(/[*_`~#>|]/g, '')                 // negrita/itálica/encabezado/código/cita/tabla
    .replace(/[ \t]{2,}/g, ' ')
    .trim();
}
