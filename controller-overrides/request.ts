...
async function resolveRequest(entry) {
  ...
  // KR_EXACT_MATCH ----------------------------------------------------------
  // Kultura: pone la cancion pedida con TOLERANCIA — typos (fuzzy) y titulos
  // parciales (por palabras). Puntua el TITULO limpio contra el texto crudo del
  // pedido. Si no hay buena coincidencia, sigue el cascade normal.
  const krNorm = (x: any) => String(x || '').toLowerCase().normalize('NFD')
    .replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
  const krClean = (t: any) => krNorm(String(t || '')
    .replace(/\([^)]*\)/g, ' ').replace(/\[[^\]]*\]/g, ' ')
    .replace(/\S*(www|mp3teca|vacilon|\.com|\.net)\S*/gi, ' '));
  const krLev = (a: string, b: string) => {
    const m = a.length, n = b.length; if (!m) return n; if (!n) return m;
    let p = Array.from({ length: n + 1 }, (_, i) => i);
    for (let i = 1; i <= m; i++) { let prev = p[0]; p[0] = i;
      for (let j = 1; j <= n; j++) { const t = p[j];
        p[j] = Math.min(p[j] + 1, p[j - 1] + 1, prev + (a[i - 1] === b[j - 1] ? 0 : 1)); prev = t; } }
    return p[n];
  };
  if (!pick) {
    const krStop = new Set(['de','del','la','el','los','las','y','con','a','feat','ft','un','una','pon','ponme','quiero','escuchar','oir','cancion','tema','reproduce','dale','por','favor','me','que','algo','esa','ese','lo']);
    const krReq = krNorm(text);
    const krReqW = krReq.split(' ').filter((w: string) => w.length >= 3 && !krStop.has(w));
    const krWM = (tw: string) => krReqW.some((rw: string) => rw === tw
      || (tw.length >= 5 && rw.length >= 5 && krLev(tw, rw) <= 2)
      || (tw.length >= 4 && rw.length >= 4 && (rw.indexOf(tw) === 0 || tw.indexOf(rw) === 0)));
    const allW = (text + ' ' + (matched.search_terms || []).join(' ') + ' ' + (matched.artist || ''))
      .split(/\s+/).map((w: string) => krNorm(w)).filter((w: string) => w.length >= 3 && !krStop.has(w));
    const queries = Array.from(new Set([text, (matched.search_terms || []).join(' '), ...allW]
      .map((q: any) => String(q || '').trim()).filter((q: string) => q)));
    let krCands: any[] = [];
    for (const q of queries) {
      try { krCands = krCands.concat(await subsonic.search(q, { songCount: 25 })); } catch (e) {}
      if (krCands.length > 250) break;
    }
    const krSeen = new Set();
    krCands = krCands.filter((x: any) => x && x.id && !krSeen.has(x.id) && krSeen.add(x.id));
    const krArtist = krNorm(matched.artist || '');
    const krScored = krCands.map((x: any) => {
      const tt = krClean(x.title), ar = krNorm(x.artist);
      const ttW = tt.split(' ').filter((w: string) => w.length >= 3 && !krStop.has(w));
      let sc = 0;
      if (tt && tt === krReq) sc = 100;
      else if (tt && tt.length >= 4 && krReq.indexOf(tt) >= 0) sc = 85;
      else if (ttW.length) {
        const m = ttW.filter(krWM).length, cov = m / ttW.length;
        if ((ttW.length === 1 && m === 1) || (m >= 2 && cov >= 0.5) || cov >= 0.7) sc = Math.round(40 + cov * 50);
      }
      if (sc > 0 && ar) {
        if (krArtist && (ar.indexOf(krArtist) >= 0 || krArtist.indexOf(ar) >= 0)) sc += 15;
        else if (krReq.indexOf(ar) >= 0) sc += 10;
      }
      return { x, sc };
    }).filter((r: any) => r.sc >= 70).sort((a: any, b: any) => b.sc - a.sc);
    if (krScored.length) { pick = krScored[0].x; pickSource = 'exact-title'; }
  }
  // KR_EXACT_MATCH end ------------------------------------------------------
