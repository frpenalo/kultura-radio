...
export async function getWeather() {
  ...
  try {
    ...
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${config.weather.lat}&longitude=${config.weather.lng}&current=temperature_2m,weather_code,is_day,precipitation,cloud_cover${unitParam}`;
    const res = await fetch(url);
    const data = await res.json() as any;
    const code = data.current.weather_code;
    let condition = mapWeatherCode(code);
    // Kultura: no anuncies lluvia/tormenta si NO esta precipitando de verdad
    // (NC en verano da codigos de chubascos del pronostico con 0 mm). Usa nubes.
    const precip = data.current.precipitation;
    if ((condition === "rainy" || condition === "stormy") && !(precip > 0)) {
      const cloud = data.current.cloud_cover;
      condition = (cloud != null && cloud >= 40) ? "cloudy" : "clear";
    }
    const result = {
      condition,
