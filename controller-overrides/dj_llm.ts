...
export async function generateIntro({ track, context, requestedBy = null, requestText = null, recap = null, recentTracks = null, recentOpeners = null }: any) {
  ...
  if (track.genre) ctxLines.push(`Genre: ${track.genre} — if you mention the style, use THIS exact genre; never invent a different one.`);
...
export async function generateLink({ previous, current, context, recap = null, recentTracks = null, recentOpeners = null }: any) {
  const ctxLines = buildContextLines(context, { recentTracks });
  // Genre for each track: prefer a real .genre tag, else parse the station's
  // album convention ("Kultura Radio - <Genre>") written by kultura_rotation.py.
  // Without this the link path carried NO genre and the DJ invented one
  // (calling salsa artists like Frankie Ruiz / Hildemaro "bachata").
  const genreOf = (t: any) => {
    if (t?.genre) return t.genre;
    const a = typeof t?.album === 'string' ? t.album : '';
    const m = a.match(/^Kultura Radio - (.+)$/);
    return m ? m[1].trim() : null;
  };
  const prevG = genreOf(previous), curG = genreOf(current);
  if (previous?.title) ctxLines.push(`Just played: "${previous.title}" by ${previous.artist || 'unknown'}${prevG ? ` — genre: ${prevG}` : ''}`);
  if (current?.title) ctxLines.push(`Now playing: "${current.title}" by ${current.artist || 'unknown'}${curG ? ` — genre: ${curG}` : ''}`);
  if (prevG || curG) ctxLines.push(`If you mention a song's musical style or genre, use ONLY the exact genre shown for that track above — never invent or guess (e.g. do NOT call salsa "bachata" or merengue "salsa"). If a track shows no genre, don't name its genre at all.`);
